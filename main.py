...
async def change(ctx, status: discord.Status, activity: str, type: str):
    await mb.Status(ctx, status, activity, type)
...
async def compteur(ctx, compteur: int):
    await others.Compteur(ctx, compteur)
...
async def kick(ctx, member: discord.Member, reason: str = None):
    await mm.Kick(ctx, member, reason)
